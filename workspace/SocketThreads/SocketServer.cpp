...
void CommunicatingSocket::shutdown()
  ...
  if (::shutdown(sockDesc, ::SHUT_RDWR) < 0) {
    throw SocketException("Shutdown failed (shutdown())", true);
  }
  if (close(sockDesc) < 0) {
    throw SocketException("Close failed (shutdown())", true);
  }
}
...
TCPSocket *TCPServerSocket::accept() throw(SocketException) {
  ...
  //    throw SocketException("Accept failed: setsockopt(SO_REUSEADDR)", true);
