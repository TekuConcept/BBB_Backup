...
uint32_t BMP085::readRawPressure(void) {
	...
	if (oversampling == IMU_ENVIRONMENT_ULTRALOWPOWER) { delay(5);  }
	else if (oversampling == IMU_ENVIRONMENT_STANDARD) { delay(8);  }
	else if (oversampling == IMU_ENVIRONMENT_HIGHRES ) { delay(14); }
	else                                               { delay(26); }
